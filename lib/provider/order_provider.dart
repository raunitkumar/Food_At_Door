...
class Orders with ChangeNotifier {
  ...
  Future<void> addOrder(List<CartItem> cartProducts, double total) async {
    final url =
        "https://fooddelivery-a27b3.firebaseio.com/orders/$_userId.json?auth=$_authToken";
    try {
          ...
          body: json.encode({
            ...
            'products': cartProducts
                .map((cp) => {
                      ...
                      'price': cp.price,
                      'title': cp.title,
                      'preprationTime': cp.preprationTime,
                    })
                .toList(),
          }));
      _orders.insert(
        ...
        _loadedOrders.add(OrderItem(
            ...
            products: (orderData['products'] as List<dynamic>)
                .map((item) => CartItem(
                      ...
                      title: item['title'],
                      preprationTime: double.parse(item['preprationTime'].toString())
                    ))
                .toList(),
